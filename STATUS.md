...
env(12+)          : SUPABASE URL/anon/service_role, APP_URL, STORIGE API_URL/KEY/EDITOR_URL,
                     PHOTOBOOK_PAGE_W/H_MM=210, TEMPLATE_SET_ID=2f312032-…(실재 셋 "sharesnap basic 210 H/C") — Vercel Production 설정됨
                     ⚠ 미적용 슬러그 photobook-210-book-4p 금지(편집기 404). 추가 판형은 Storige Admin 등록 UUID 사용
...
| I-404 | 높음 | 편집기 "편집기를 열지 못했어요 / 템플릿셋을 불러올 수 없습니다(photobook-210-book-4p) 404" — env STORIGE_TEMPLATE_SET_ID가 **미적용 시드 슬러그**를 가리킴(Storige DB에 없음). createEditSession은 templateSetId를 검증 없이 저장만 해 세션생성 201이라 앞선 "검증"이 이를 못 잡음(편집기 화면 미확인) | ✅ 해결(2026-06-22) | env를 **실재 셋 `2f312032…`(sharesnap basic 210 H/C)** 로 교체+재배포. 깨진 템플릿ID 저장된 기존 세션 2건 storige_session_id=NULL로 무효화(재생성 유도). 편집기 로드(editor.ready, 404 없음) E2E 확인 |
