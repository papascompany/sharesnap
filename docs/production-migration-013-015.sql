...
-- ============================================================
-- ⚠ PostgREST 스키마 캐시 리로드 (필수!)
--   SQL Editor는 DB에 직접 연결하므로 아래 검증 쿼리는 통과하지만,
--   앱이 쓰는 REST API(PostgREST)는 스키마 캐시가 자동 갱신되기 전까지
--   새 테이블(profiles/reports)·컬럼(tracking_*)·RPC를 404/400으로 응답한다.
--   이 NOTIFY 한 줄이 캐시를 즉시 리로드해 앱에서 바로 동작하게 한다.
--   (자동 갱신을 기다려도 되지만 지연·누락될 수 있어 명시적으로 호출한다)
-- ============================================================
